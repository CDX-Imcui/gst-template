...
gst_undistort_class_init (GstUndistortClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_FX,
    g_param_spec_double ("fx", "fx", "Focal length fx (pixels)", 0.0, G_MAXDOUBLE, 0.0, G_PARAM_READWRITE));
  g_object_class_install_property (gobject_class, PROP_FY,
    g_param_spec_double ("fy", "fy", "Focal length fy (pixels)", 0.0, G_MAXDOUBLE, 0.0, G_PARAM_READWRITE));
  g_object_class_install_property (gobject_class, PROP_CX,
    g_param_spec_double ("cx", "cx", "Principal point cx", 0.0, G_MAXDOUBLE, 0.0, G_PARAM_READWRITE));
  g_object_class_install_property (gobject_class, PROP_CY,
    g_param_spec_double ("cy", "cy", "Principal point cy", 0.0, G_MAXDOUBLE, 0.0, G_PARAM_READWRITE));
  g_object_class_install_property (gobject_class, PROP_K1,
    g_param_spec_double ("k1", "k1", "Radial distortion k1", -10.0, 10.0, 0.0, G_PARAM_READWRITE));
  g_object_class_install_property (gobject_class, PROP_K2,
    g_param_spec_double ("k2", "k2", "Radial distortion k2", -10.0, 10.0, 0.0, G_PARAM_READWRITE));
  g_object_class_install_property (gobject_class, PROP_P1,
    g_param_spec_double ("p1", "p1", "Tangential distortion p1", -10.0, 10.0, 0.0, G_PARAM_READWRITE));
  g_object_class_install_property (gobject_class, PROP_P2,
    g_param_spec_double ("p2", "p2", "Tangential distortion p2", -10.0, 10.0, 0.0, G_PARAM_READWRITE));
  g_object_class_install_property (gobject_class, PROP_K3,
    g_param_spec_double ("k3", "k3", "Radial distortion k3", -10.0, 10.0, 0.0, G_PARAM_READWRITE));
...
gst_undistort_set_property (GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_SILENT: self->silent = g_value_get_boolean (value); break;
    case PROP_FX: self->fx = g_value_get_double (value); break;
    case PROP_FY: self->fy = g_value_get_double (value); break;
    case PROP_CX: self->cx = g_value_get_double (value); break;
    case PROP_CY: self->cy = g_value_get_double (value); break;
    case PROP_K1: self->k1 = g_value_get_double (value); break;
    case PROP_K2: self->k2 = g_value_get_double (value); break;
    case PROP_P1: self->p1 = g_value_get_double (value); break;
    case PROP_P2: self->p2 = g_value_get_double (value); break;
    case PROP_K3: self->k3 = g_value_get_double (value); break;
    default:
...
gst_undistort_get_property (GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
  GstUndistort *self = GST_UNDISTORT (object);
  switch (prop_id) {
    case PROP_SILENT: g_value_set_boolean (value, self->silent); break;
    case PROP_FX: g_value_set_double (value, self->fx); break;
    case PROP_FY: g_value_set_double (value, self->fy); break;
    case PROP_CX: g_value_set_double (value, self->cx); break;
    case PROP_CY: g_value_set_double (value, self->cy); break;
    case PROP_K1: g_value_set_double (value, self->k1); break;
    case PROP_K2: g_value_set_double (value, self->k2); break;
    case PROP_P1: g_value_set_double (value, self->p1); break;
    case PROP_P2: g_value_set_double (value, self->p2); break;
    case PROP_K3: g_value_set_double (value, self->k3); break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
  }
}
...
/* 帧内就地处理：把 frame 映射成 cv::Mat，remap 到临时图，再拷回 */
...
gst_undistort_transform_frame_ip (GstVideoFilter * filter, GstVideoFrame * frame)
{
  ...
  cv::remap(img, priv->scratch, priv->mapx, priv->mapy, cv::INTER_LINEAR);
